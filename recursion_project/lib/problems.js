...
function flatten(data) { // [1,2,[3,3]]    i = 0
...
function fileFinder(directories, targetFile) {
    ...
    result = false;
    if (directories[targetFile] === undefined) result = false;
    let directoriesArr = Object.keys(directories);
    for (let i = 0; i < directoriesArr.length; i++) {
        const directory = directoriesArr[i]
        result = fileFinder(directories[directory], targetFile)
...
function pathFinder(directories, targetFile) {
    ...
    if (directories[targetFile] === null) return `/${targetFile}`;
    if (directories[targetFile] === undefined) filePath = '';


    let directoriesArr = Object.keys(directories);
    for (let i = 0; i < directoriesArr.length; i++) {
        let directory = directoriesArr[i];
        filePath = pathFinder(directories[directory], targetFile);
